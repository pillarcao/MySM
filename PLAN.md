...
- [x] Event Logging：EVTLOG 表自动记录所有操作审计日志
- [x] Logging 菜单：按日期/表名/用户搜索事件日志
- [x] Logout API：退出时记录 Logoff 事件

### UI 工业风格验证

- [x] 冷色调色板：深钴蓝主色 #2B5CE6，冷灰背景/边框
- [x] 硬朗线条：全局 0 圆角，2px 实线分隔，主色强调线
- [x] 高信息密度：表格铺满中间面板，去掉卡片留白
- [x] 分区清晰：左/中/右面板 2px 硬边界，标题栏蓝色底线
- [x] 行状态色彩：Edit(白+蓝条) / EditComp(暖黄+琥珀条) / Released(浅绿+绿条)
- [x] 选中/Hover 行：黑底白字，视觉焦点明确
- [x] 登录页：深色全屏+网格纹理，方正表单
- [x] 单元格不换行，溢出省略+tooltip
...
1. **Report 功能迁移**：Route/MRoute/Operation/Product 层级报表（依赖 D 表有实际数据）
2. **DB2 对接**：将 H2 schema/data 迁移到真实 DB2 环境，验证 SQL 语法兼容性
3. **复杂表回归测试**：选 BPROD（字段多、含 Dummy Field）做完整端到端验证
4. **生产加固**：并发锁冲突检测、Session 超时自动清理
5. **灰度切流方案**：设计 Nginx 路由规则，支持按用户/按表逐步从 MFC 切到 Web
